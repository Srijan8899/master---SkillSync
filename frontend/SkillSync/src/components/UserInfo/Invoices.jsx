...
const Invoices = () => {
  ...
  useEffect(() => {
    const fetchInvoices = async () => {
      try {
        const response = await fetch("http://localhost:8000/api/user/invoice", {
          headers: {
            method: "GET",
            ...
            <div className="max-w-full overflow-x-auto">
              <table className="w-full table-auto">
                ...
                <tbody>
                  {invoices.length === 0 ? (
                    <tr>
                      <td colSpan="6" className=" text-gray-700 w-full h-full col-span-5 text-center text-5xl my-10">No invoice records found</td>
                    </tr>
                  ) : (
